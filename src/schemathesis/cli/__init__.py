...
def execute(
    ...
    try:
        for event in prepared_runner:
            for handler in handlers:
                # Do not raise an error there?
                handler.handle_event(context, event)
    finally:
        # Finalization might be slow, writing to a file should be in a separate thread and in a streaming fashion
        # Also, the final stdout line probably should be after finalization
        for handler in handlers:
            handler.finalize()
